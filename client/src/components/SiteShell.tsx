...
export function SiteShell({ children }: { children: ReactNode }) {
  ...
  return <div className="min-h-screen overflow-x-hidden bg-[#090909] text-[#f4f0e9]">
    ...
    <footer className="border-t border-white/10 py-12"><div className="lumen-container flex flex-col justify-between gap-10 md:flex-row"><div><div className="mb-3 text-sm font-extrabold tracking-[.18em]">إنتاج<span className="text-[#d6b17d]">/</span>دعاية وإشهار</div><p className="max-w-xs text-sm leading-7 text-[#a6a19a]">{t("footer")}</p></div><div className="flex gap-6 text-sm text-[#a6a19a]"><a href="https://instagram.com" target="_blank" rel="noreferrer">Instagram</a><a href="https://vimeo.com" target="_blank" rel="noreferrer">Vimeo</a><Link href="/admin">{t("admin")}</Link></div></div><div className="lumen-container mt-10 flex justify-between border-t border-white/10 pt-5 text-[10px] uppercase tracking-[.16em] text-[#6f6b66]"><span>{t("copyright")}</span><span>{t("directionNote")}</span></div></footer>
  </div>;
}
