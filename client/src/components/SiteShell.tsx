...
export function SiteShell({ children }: { children: ReactNode }) {
  ...
  const [settings, setSettings] = useState<{ email?: string; instagram_url?: string; linkedin_url?: string; behance_url?: string } | null>(null);
  ...
  return <div className="min-h-screen overflow-x-hidden bg-[#090909] text-[#f4f0e9]">
    ...
    <footer className="border-t border-white/10 py-12"><div className="lumen-container flex flex-col justify-between gap-10 md:flex-row"><div><div className="mb-3 text-sm font-extrabold tracking-[.18em]">إنتاج<span className="text-[#d6b17d]">/</span>دعاية وإشهار</div><p className="max-w-xs text-sm leading-7 text-[#a6a19a]">{t("footer")}</p>{settings?.email&&<a href={`mailto:${settings.email}`} className="mt-4 inline-block text-xs text-[#d6b17d]">{settings.email}</a>}</div><div className="flex flex-wrap gap-6 text-sm text-[#a6a19a]">{settings?.instagram_url&&<a href={settings.instagram_url} target="_blank" rel="noreferrer">Instagram</a>}{settings?.linkedin_url&&<a href={settings.linkedin_url} target="_blank" rel="noreferrer">LinkedIn</a>}{settings?.behance_url&&<a href={settings.behance_url} target="_blank" rel="noreferrer">Behance</a>}{showAdminEntry&&<Link href="/admin/login">{t("admin")}</Link>}</div></div><div className="lumen-container mt-10 flex justify-between border-t border-white/10 pt-5 text-[10px] uppercase tracking-[.16em] text-[#6f6b66]"><span>{t("copyright")}</span><span>{t("directionNote")}</span></div></footer>
  </div>;
}
